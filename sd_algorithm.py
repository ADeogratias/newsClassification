...
class SDAlgorithm():
    ...
    def analyze_page(self):
        ...
        article, comments, multiple, mytext = self.classify_page()
        print(article)

        if article is not None and comments is None:
            return 'article', mytext, None, None
        elif article is not None:
            return 'comment', article, comments
        else:
            return 'multiple', None, None, mytext
    ...
    def classify_page(self):
        ...
        for reg in biggest_regions:
           #print (reg.full_text)
           mytext.append(reg.full_text)
        print("Successfully mined data")
        return None, None, biggest_regions, mytext
    ...
    def group_regions(self):
        ...
        for region in self.regions: 
            ...
            pr_com = (len(region.tree.xpath(region.root)) > 0 and\
            	      region.tree.xpath(region.root)[0].getparent().attrib.__contains__('class') and \
            ...
            if region.distance_from_max != 0 and (region.class_name != "" or \
               (region.class_name == "" and pr_com)):
                if not grouped_comments.__contains__(region.class_name):
    ...
    def big_areas_in_same_level(self, article, grouped_comments, max_group):
        ...
        HTML tree structure.
        """
        if grouped_comments.__contains__(max_group):
    ...
    def candidate_context_validated(self, article, grouped_comments, max_group):
        ...
        for des in list(comment_parent.iterdescendants()) + [comment_parent]:
            classname = id = ""
            if des.attrib.__contains__("class"):
                classname = des.attrib['class']
            if des.attrib.__contains__("id"):
                id = des.attrib['id']
    ...
    def merge_groups(self, tree):
        ...
        for group in tmp_nodes:
            ...
            if parent is not None:
                ...
                if self.valid_nodes.__contains__(parent_path):
    ...
    def cross_tree(self, node, node_text=None, level=0):
        ...
        if node.attrib.__contains__("class") and node.attrib["class"] == "wrappers":
            dess = []
    ...
    def get_style(self, node):
        ...
        """
        if node.attrib.__contains__("style"):
    ...
    def group_node(self, node, node_text):
        ...
        if parent_path not in ["/html","/html/body"] and node_text is not None\
           and node.tag != 'body' and self.has_visible_parents(valid_parent):
            if not self.valid_nodes.__contains__(parent_path):
...
if __name__ == '__main__': 
    url = 'https://www.nytimes.com/2021/03/29/arts/music/sasha-velour-drag-opera.html'
    sd = SDAlgorithm()
    sd.url = url
